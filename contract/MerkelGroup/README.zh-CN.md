...
> `MerkelGroup` 目录名为兼容既有链接而保留。当前协议模型是本文描述的 BaseCommunity 架构，不再是早期的 clone + Room 设计。
...
本目录保留的 `Community.sol`、`CommunityFactory.sol` 和 `Room.sol` 是早期 EIP-1167 clone + Room 设计的历史文档快照。它们不代表当前社群架构，也不能作为新集成的 source-of-truth。
